...
namespace {
  ...
  template<Tracing T>
  Score Evaluation<T>::evaluate_initiative(Value mg, Value eg) {
    ...
    int vM = ((mg > 0) - (mg < 0)) * std::max(4 * pe->open_files() - 16, -abs(mg/2));
  ...
  Value Evaluation<T>::value() {
    ...
    score += evaluate_initiative(mg_value(score), eg_value(score));
