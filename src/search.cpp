...
namespace {
  ...
  int stat_bonus(Depth depth, Depth rootDepth) {
    int d = (depth - std::max(rootDepth - 18 * ONE_PLY, DEPTH_ZERO)) / ONE_PLY;
    return (d < 1 || d > 17) ? 0 : 32 * d * d + 64 * d - 64;
  }
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                if (!pos.capture_or_promotion(ttMove))
                    update_quiet_stats(pos, ss, ttMove, nullptr, 0, stat_bonus(depth, thisThread->rootDepth));
            ...
            {
                int penalty = -stat_bonus(depth, thisThread->rootDepth);
    ...
    */
    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_quiet_stats(pos, ss, bestMove, quietsSearched, quietCount, stat_bonus(depth, thisThread->rootDepth));
        else
            update_capture_stats(pos, bestMove, capturesSearched, captureCount,
                                 stat_bonus(depth + (bestValue > beta + KnightValueMg ? ONE_PLY : DEPTH_ZERO), thisThread->rootDepth));
    ...
    // Bonus for prior countermove that caused the fail low
        ...
        update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, stat_bonus(depth, thisThread->rootDepth));
